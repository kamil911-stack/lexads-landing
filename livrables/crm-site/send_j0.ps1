...
.SYNOPSIS
  ...
  Prerequis : IONOS_FROM_EMAIL, IONOS_PASSWORD definis au niveau utilisateur.
#>
...
foreach ($p in $toSend) {
  ...
  # Nouveau client SMTP par email — evite le bug de connexion reutilisee avec IONOS STARTTLS
...
# ── RAPPORT ──────────────────────────────────────────────────────────────────
$koList = @($sentLog | Where-Object { !$_.Ok })
Write-Host "`n========================================"
Write-Host "RAPPORT J0 — $(Get-Date -Format 'dd/MM/yyyy HH:mm')"
Write-Host "  OK     : $okCount / $total"
if ($koList.Count -gt 0) {
  Write-Host "  Echecs ($($koList.Count)) :" -ForegroundColor Red
  $koList | ForEach-Object {
